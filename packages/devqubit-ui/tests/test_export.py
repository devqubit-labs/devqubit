...
class TestCreateExport:
    ...
    def test_creates_bundle(self, client, mock_run_record):
    ...
    def test_run_not_found(self, client, mock_registry):
        ...
        resp = client.post("/api/runs/missing/export")
...
class TestDownloadExport:
    ...
    def test_download_after_create(self, client, mock_run_record):
    ...
    def test_download_without_create(self, client, mock_registry):
        ...
        resp = client.get("/api/runs/no-bundle/export/download")
        assert resp.status_code == 404
...
class TestExportInfo:
    ...
    def test_info_returns_counts(self, client, mock_run_record):
    ...
    def test_run_not_found(self, client, mock_registry):
        mock_registry.load.side_effect = KeyError("gone")
        resp = client.get("/api/runs/missing/export/info")
        assert resp.status_code == 404
...
class TestCleanupExport:
    ...
    def test_cleanup_after_create(self, client, mock_run_record):
        ...
        resp = client.get("/api/runs/run-clean/export/download")
        assert resp.status_code == 404

    def test_cleanup_noop_when_missing(self, client):
        resp = client.delete("/api/runs/no-such-run/export")
        assert resp.json()["status"] == "cleaned"
...
class TestPathTraversal:
    ...
    def test_valid_ids_accepted(self, client, mock_run_record):
